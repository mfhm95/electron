...
class HrLoan(models.Model):
    ...
    payment_date = fields.Date(string="Payment Start Date", required=True, default=fields.Date.today(), tracking=True, help="Date of "
                                                                                                             "the "
                                                                                                             "paymemt")
    ...
    total_balance = fields.Float('Total Emp. Balance', compute='_compute_total_balance')
    ...
    def _compute_total_balance(self):
        for loan in self:
            ...
            loan.total_balance = total_balance
    ...
    state = fields.Selection([
        ...
        ('refuse', 'Refused'),
        ('cancel', 'Canceled'),
    ], string="State", default='draft', track_visibility='onchange', copy=False, tracking=True)
            ...
            for user in self.env.ref('hr.group_hr_manager').users:
                data = {
                    'res_model_id': self.env['ir.model'].search(
                        [('model', '=', 'hr.loan')]).id,
                    'res_id': rec.id,
                    'activity_type_id': self.env[
                        'mail.activity.type'].search(
                        [('name', 'like', 'To Do')]).id,
                    'summary': "Loan Request",
                    'note': "there is a loan request waiting your approve",
    ...
    def action_cancel(self):
        if self.loan_entry_id:
            self.loan_entry_id.state = 'cancel'
