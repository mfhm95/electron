...
class HrLoanAcc(models.Model):
    ...
    def _check_is_dept_manager(self):
        for loan in self:
            if loan.department_id and loan.department_id.manager_id and \
                    loan.department_id.manager_id.user_id and \
                    loan.department_id.manager_id.user_id.id == self.env.uid:
                loan.is_dept_manager = True
            else:
                loan.is_dept_manager = False

    state = fields.Selection([
        ('draft', 'Submitted'),
        ('waiting_approval_1', 'HR Approval'),
        ('waiting_approval_2', 'Account Approval'),
        ('approve', 'Paid'),
        ('refuse', 'Refused'),
        ('cancel', 'Canceled'),
    ...
    def action_approve(self):
        ...
        if loan_approve:
            ...
            for loan in self:
                ...
                move.post()
                loan.state = 'approve'
                loan.loan_entry_id = move.id
                loan.write({'state': 'approve', 'loan_entry_id': move.id})
        return True
